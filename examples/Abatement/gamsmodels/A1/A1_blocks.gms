...
$BLOCK M_T_out 
	...
	E_q_out_T_out[n]$(bra_o_T_out[n])..	qS[n] =E= sum(nn$(map_T_out[n,nn]), mu[n,nn] * (PbT[n]/PwT[nn])**(-eta[nn]) * qD[nn]);
	E_q_nout_T_out[n]$(bra_no_T_out[n])..	qD[n] =E= sum(nn$(map_T_out[n,nn]), mu[n,nn] * (PwT[n]/PwT[nn])**(-eta[nn]) * qD[nn]);
$ENDBLOCK
$BLOCK M_C 
...
$BLOCK M_E 
	...
	E_q_out_E[n]$(bra_o_E[n])..	qD[n] =E= sum(nn$(map_E[n,nn]), mu[n,nn] * (PbT[nn]/PwT[n])**(sigma[nn]) * qS[nn]);
	E_q_nout_E[n]$(bra_no_E[n])..	qD[n] =E= sum(nn$(map_E[n,nn]), mu[n,nn] * (PwT[nn]/PwT[n])**(sigma[nn]) * qD[nn]);
$ENDBLOCK
$BLOCK M_A1_pw 
	E_pw_A1[n]$(out[n])..	Peq[n] =E= (1+markup[n])*(PbT[n]+tauS[n]+0);
$ENDBLOCK
