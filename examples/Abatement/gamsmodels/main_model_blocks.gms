...
$BLOCK M_ID_CU 
	...
	E_pout_ID_CU[n]$(ID_out_ID_CU[n])..	PbT[n] =E= sum(nn$(map_ID_CU[nn,n]), mu[nn,n]*share[nn,n]*PwThat[nn]);
	E_pnout_ID_CU[n]$(kno_no_ID_CU[n])..	PwThat[n] =E= sum(nn$(map_ID_CU[nn,n]), mu[nn,n]*share[nn,n]*PwThat[nn]);
...
$BLOCK M_ID_TU 
	...
	E_qout_ID_TU[n]$(bra_o_ID_TU[n] and ID_q_unique[n])..	qS[n] =E= sum(nn$(map_ID_TU[n,nn]), share[n,nn]*(qD[nn]-epsi));
	E_qnout_ID_TU[n]$(bra_no_ID_TU[n] and ID_q_unique[n])..	qD[n] =E= sum(nn$(map_ID_TU[n,nn]), share[n,nn]*(qD[nn]-epsi))+epsi;
$ENDBLOCK
$BLOCK M_ID_TX 
...
$BLOCK M_EOP_CU 
	...
	E_pout_EOP_CU[n]$(EOP_out_EOP_CU[n])..	PbT[n] =E= sum(nn$(map_EOP_CU[nn,n]), mu[nn,n]*share[nn,n]*PwThat[nn]);
	E_pnout_EOP_CU[n]$(kno_no_EOP_CU[n])..	PwThat[n] =E= sum(nn$(map_EOP_CU[nn,n]), mu[nn,n]*share[nn,n]*PwThat[nn]);
...
$BLOCK M_EOP_TU 
	...
	E_qout_EOP_TU[n]$(bra_o_EOP_TU[n] and EOP_q_unique[n])..	qS[n] =E= sum(nn$(map_EOP_TU[n,nn]), share[n,nn]*(qD[nn]-epsi));
	E_qnout_EOP_TU[n]$(bra_no_EOP_TU[n] and EOP_q_unique[n])..	qD[n] =E= sum(nn$(map_EOP_TU[n,nn]), share[n,nn]*(qD[nn]-epsi))+epsi;
$ENDBLOCK
$BLOCK M_EOP_TX 
...
$BLOCK M_main_model_EOP_Em 
	E_EOP_qS_main_model[n]$(EOP_out[n])..	qS[n] =E= sum(z$(m2c[z,n]), M0[z]*theta[z,n]*errorf((pM[z]-PbT[n]+muG[n])/sigmaG[n]));
	E_EOP_M_main_model[z]..	M[z] =E= M0[z]-sum(n$(m2c[z,n]), qS[n]);
