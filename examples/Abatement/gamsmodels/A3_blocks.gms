...
$BLOCK M_EOP_TU 
	...
	E_qout_EOP_TU[n]$(bra_o_EOP_TU[n])..	qS[n] =E= sum(nn$(map_EOP_TU[n,nn]), share[n,nn]*(qD[nn]-epsi));
	E_qnout_EOP_TU[n]$(bra_no_EOP_TU[n])..	qD[n] =E= sum(nn$(map_EOP_TU[n,nn]), share[n,nn]*(qD[nn]-epsi))+epsi;
$ENDBLOCK
$BLOCK M_EOP_TX 
