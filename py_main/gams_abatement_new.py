...
class CES:
	...
	def __init__(self,version='std',state=None,**kwargs):
		...
		self.state = state
	def add_symbols(self,db,ns,ns_global={},dynamic=False):
		[setattr(self,sym,db[df(sym,ns)]) for sym in ['map_']]; # local symbols
		[setattr(self,sym,db[df(sym,ns_global)]) for sym in ('n','PbT','PwThat','mu','sigma','share','epsi','qS','qD')];
		if dynamic is True:
			[setattr(self,sym,db[df(sym,ns_global)]) for sym in ('txE','t0','tE','tx0E')];
	...
	def a(self,attr,lot_indices=[],l='',lag={}):
		return getattr(self,attr).write(alias=create_alias_dict(self.aliases,lot_indices),l=l,lag=lag) if isinstance(attr,str) else attr.write(alias=create_alias_dict(self.aliases,lot_indices),l=l,lag=lag)
	def add_conditions(self,db,ns_tree,dynamic=False):
		self.conditions = {'p_out': self.a(db[ns_tree['tree_out']]), 'p_nout': self.a(db[ns_tree['kno_no']]),
							's_out': f"{self.a(db[ns_tree['map_']])} and {self.a(db[ns_tree['bra_o']])}", 
							's_nout': f"{self.a(db[ns_tree['map_']])} and {self.a(db[ns_tree['bra_no']])}",
							'q_out': self.a(db[ns_tree['bra_o']]), 'q_nout': self.a(db[ns_tree['bra_no']])}
		if dynamic is True:
			self.conditions = {key: value+' and '+self.txE.write() for key,value in self.conditions.items()}
	def run(self,name):
		nn, map_, map_2 = self.a('n',[(0,1)]), self.a('map_'), self.a('map_',[(0,1),(1,0)])
		mu, sigma2 = self.a('mu'), self.a('sigma',[(0,1)])
	...
	def e_price(self,name,conditions,nn,map_2,share_2,PwThat2,output=True):
		...
		if output is True:
			return equation(name,self.PbT.doms(),conditions,self.PbT.write(),RHS)
		else:
			return equation(name,self.PwThat.doms(),conditions,self.PwThat.write(),RHS)
	def e_quant(self,name,conditions,nn,map_,share,qS2,qD2,epsi,output=False):
		RHS = f"sum({nn}$({map_}), {share}*{qS2})+{epsi}" if output is True else f"sum({nn}$({map_}), {share}*({qD2}-{epsi}))+{epsi}"
		return equation(name,self.qD.doms(),conditions,self.qD.write(),RHS)
...
class CES_norm:
	...
	def run(self,name):
		...
		mu,mu_3, sigma2 = self.a('mu'),self.a('mu',[(0,2)]),self.a('sigma',[(0,1)])
...
class CET:
	...
	def add_symbols(self,db,ns,ns_global={},dynamic=False):
		[setattr(self,sym,db[df(sym,ns)]) for sym in ['map_']]; # local symbols
		[setattr(self,sym,db[df(sym,ns_global)]) for sym in ('n','PbT','PwThat','mu','eta','share','epsi','qS','qD')];
		[setattr(self,sym,db[df(self.state+'_'+sym,ns_global)]) for sym in ['out','q_unique']]; # state-specific symbols.
		if dynamic is True:
			[setattr(self,sym,db[df(sym,ns_global)]) for sym in ('txE','t0','tE','tx0E')];
		self.aliases = {i: db.alias_dict0[self.n.name][i] for i in range(len(db.alias_dict0[self.n.name]))}
	...
	def run(self,name):
		nn,map_,map_2 = self.a('n',[(0,1)]), self.a('map_'),self.a('map_',[(0,1),(1,0)])
		mu,eta2 = self.a('mu'),self.a('eta',[(0,1)])
		PwThat,PwThat2,PbT,PbT2 = self.a('PwThat'),self.a('PwThat',[(0,1)]),self.a('PbT'),self.a('PbT',[(0,1)])
		share,share_2,epsi = self.a('share'),self.a('share',[(0,1),(1,0)]),self.a('epsi')
	...
	def e_quant(self,name,conditions,nn,map_,share,epsi,qD2,output=True):
		if output is True:
			RHS = f"sum({nn}$({map_}), {share}*({qD2}-{epsi}))"
			return equation(name,self.qS.doms(),conditions,self.qS.write(),RHS)
		else: 
			RHS = f"sum({nn}$({map_}), {share}*({qD2}-{epsi}))+{epsi}"
			return equation(name,self.qD.doms(),conditions,self.qD.write(),RHS)
...
class CET_norm:
	...
	def run(self,name):
		nn,nnn = self.a('n',[(0,1)]), self.a('n',[(0,2)])
		map_,map_2,map_3 = self.a('map_'),self.a('map_',[(0,1),(1,0)]),self.a('map_',[(0,2)])
		mu,mu3,eta2 = self.a('mu'),self.a('mu',[(0,2)]), self.a('eta',[(0,1)])
...
class MNL:
	...
	def run(self,name):
		nn,nnn = self.a('n',[(0,1)]), self.a('n',[(0,2)])
		map_,map_2,map_3=self.a('map_'),self.a('map_',[(0,1),(1,0)]), self.a('map_',[(0,2)])
		mu,mu_2,mu_3, sigma2 = self.a('mu'),self.a('mu',[(0,1),(1,0)]),self.a('mu',[(0,2)]),self.a('sigma',[(0,1)])
		PwThat,PwThat2,PwThat3,PbT2 = self.a('PwThat'),self.a('PwThat',[(0,1)]), self.a('PwThat',[(0,2)]),self.a('PbT',[(0,1)])
		qS2,qD2 = self.a('qS',[(0,1)]), self.a('qD',[(0,1)])
		share,share_2,epsi = self.a('share'),self.a('share',[(0,1),(1,0)]),self.a('epsi')
		text = self.e_share(f"E_sout_{name}",self.conditions['s_out'],nnn,map_3,mu,mu_3,PwThat,PwThat2,PwThat3,PbT2,sigma2,output=True)+'\n\t'
		text += self.e_share(f"E_snout_{name}",self.conditions['s_nout'],nnn,map_3,mu,mu_3,PwThat,PwThat2,PwThat3,PbT2,sigma2,output=False)+'\n\t'
		text += self.e_price(f"E_pout_{name}",self.conditions['p_out'],nn,map_2,mu_2,share_2,PwThat2,output=True)+'\n\t'
		text += self.e_price(f"E_pnout_{name}",self.conditions['p_nout'],nn,map_2,mu_2,share_2,PwThat2,output=False)+'\n\t'
		text += self.e_quant(f"E_qout_{name}", self.conditions['q_out'],nn,map_,share,qS2,qD2,epsi,output=True)+'\n\t'
		text += self.e_quant(f"E_qnout_{name}",self.conditions['q_nout'],nn,map_,share,qS2,qD2,epsi,output=False)
		return text
	...
	def e_price(self,name,conditions,nn,map_2,mu_2,share_2,PwThat2,output=True):
		RHS = f"""sum({nn}$({map_2}), {mu_2}*{share_2}*{PwThat2})"""
		if output is True:
			return equation(name,self.PbT.doms(),conditions,self.PbT.write(),RHS)
		else:
			return equation(name,self.PwThat.doms(),conditions,self.PwThat.write(),RHS)
...
class linear_out:
	...
	def add_symbols(self,db,ns,ns_global={},dynamic=False):
		[setattr(self,sym,db[df(sym,ns)]) for sym in ['map_']]; # local symbols
		[setattr(self,sym,db[df(sym,ns_global)]) for sym in ('n','PbT','PwThat','mu','share','qS','qD')];
		[setattr(self,sym,db[df(self.state+'_'+sym,ns_global)]) for sym in ['out']]; # state-specific symbols.
		if dynamic is True:
			[setattr(self,sym,db[df(sym,ns_global)]) for sym in ('txE','t0','tE','tx0E')];
		self.aliases = {i: db.alias_dict0[self.n.name][i] for i in range(len(db.alias_dict0[self.n.name]))}

	def add_conditions(self,db,ns_tree,dynamic=False):
		self.conditions = {'p_out': self.a(db[ns_tree['bra_o']]), 'p_nout': self.a(db[ns_tree['bra_no']]),
						   's_out': f"{self.a(db[ns_tree['map_']])} and {self.a(db[ns_tree['bra_o']])}",
						   's_nout': f"{self.a(db[ns_tree['map_']])} and {self.a(db[ns_tree['bra_no']])}",
						   'q': self.a(db[ns_tree['knots']])}
		if dynamic is True:
			self.conditions = {key: value+' and '+self.txE.write() for key,value in self.conditions.items()}
	...
	def run(self,name):
		...
		mu,mu_2 = self.a('mu'), self.a('mu',[(0,1),(1,0)])
		share,out_2 = self.a('share'),self.a('out',[(0,1)])
		qD,qD2,qS,qS2 = self.a('qD'),self.a('qD',[(0,1)]),self.a('qS'),self.a('qS',[(0,1)])
		PwThat2 = self.a('PwThat',[(0,1)])
		text = self.e_price(f"E_pout_{name}", self.conditions['p_out'],nn,map_,mu,PwThat2,output=True)+'\n\t'
		text += self.e_price(f"E_pnout_{name}", self.conditions['p_nout'],nn,map_,mu,PwThat2,output=False)+'\n\t'
		text += self.e_share(f"E_sout_{name}", self.conditions['s_out'],share,qD,qD2,qS,output=True)+'\n\t'
		text += self.e_share(f"E_snout_{name}", self.conditions['s_nout'],share,qD,qD2,qS,output=False)+'\n\t'
		text += self.e_quant(f"E_q_{name}", self.conditions['q'],nn,map_2,out_2,mu_2,qS2,qD2)
		return text

	def e_share(self,name,conditions,share,qD,qD2,qS,output=True):
		if output is True:
			return equation(name,self.share.doms(),conditions,self.share.write(),f"{qS}/{qD2}")
		else:
			return equation(name,self.share.doms(),conditions,self.share.write(),f"{qD}/{qD2}")

	def e_price(self,name,conditions,nn,map_,mu,PwThat2,output=True):
		if output is True:
			return equation(name,self.PbT.doms(),conditions,self.PbT.write(),f"sum({nn}$({map_}),{mu}*{PwThat2})")
		else:
			return equation(name,self.PwThat.doms(),conditions,self.PwThat.write(),f"sum({nn}$({map_}),{mu}*{PwThat2})")

	def e_quant(self,name,conditions,nn,map_2,out_2, mu_2,qS2,qD2):
		RHS = f"""sum({nn}$({map_2} and {out_2}), {qS2}/{mu_2})+sum({nn}$({map_2} and not {out_2}), {qD2}/{mu_2})"""
		return equation(name,self.qD.doms(),conditions,self.qD.write(),RHS)
...
class ID_emissions:
	def __init__(self):
		pass
	def add_symbols(self,db,ns):
		[setattr(self,sym,db[df(sym,ns)]) for sym in ('n','z','ai','ID_inp','phi','qD','M0','PwT','PwThat','pMhat', "ID_i2ai")];
		self.aliases = {i: db.alias_dict0[self.n.name][i] for i in range(len(db.alias_dict0[self.n.name]))}
	def add_conditions(self):
...
class aggregates:
	...
	def add_symbols(self,db,ns):
		if self.state == 'ID':
			[setattr(self,sym,db[df(sym,ns)]) for sym in ('n','z','ai','ID_i2ai','qD','pM','pMhat')];
		elif self.state == 'EOP':
			[setattr(self,sym,db[df(sym,ns)]) for sym in ('n','z','ai','ID_i2ai','EOP_i2ai','m2c','qD','pM','pMhat','PbT','theta','muG','sigmaG')];
		self.aliases = {i: db.alias_dict0[self.n.name][i] for i in range(len(db.alias_dict0[self.n.name]))}
	def add_conditions(self):
...
class minimize_object:
	def __init__(self, state = 'ID'):
		self.state = state
	def add_symbols(self,db,ns):
		[setattr(self,sym,db[df(sym,ns)]) for sym in ('n','map_gamma','map_ID_CU','bra_ID_TU','mu','mubar','weight_mu','gamma_tau','minobj')];
		if self.state == 'EOP':
			[setattr(self,sym,db[df(sym,ns)]) for sym in ('kno_EOP_CU','muG','sigmaG','w_EOP','w_mu_EOP','muGbar','sigmaGbar')];
		self.aliases = {i: db.alias_dict0[self.n.name][i] for i in range(len(db.alias_dict0[self.n.name]))}
	def add_conditions(self):
...
class currapp_EOP:
	def __init__(self):
		pass
	def add_symbols(self,db,ns):
		[setattr(self,sym,db[df(sym,ns)]) for sym in ('n','m2u','m2t','map_EOP_TU','qD','M0','currapp_EOP')];
		self.aliases = {i: db.alias_dict0[self.n.name][i] for i in range(len(db.alias_dict0[self.n.name]))}
	def add_conditions(self):
...
class EOP_emissions:
	def __init__(self):
		pass
	def add_symbols(self,db,ns):
		[setattr(self,sym,db[df(sym,ns)]) for sym in ('n','z','m2c','EOP_out','EOP_inp','EOP_i2ai','theta','phi','qS','qD','M0','M','pM','pMhat','PbT','PwT','PwThat','muG','sigmaG','epsi')];
		self.aliases = {i: db.alias_dict0[self.n.name][i] for i in range(len(db.alias_dict0[self.n.name]))}
	def add_conditions(self):
